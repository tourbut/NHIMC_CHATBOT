...
def refine_input_prompt(document_metadata):
    ...
    system_prompt += "{document_metadata}\n"
    system_prompt += "</Document Metadata>\n\n"
    system_prompt += "- Extract key information\n\n"
    ...
    system_prompt += "3. **Must Use Retrieve Tool**\n"
    system_prompt += "- You must use the retrieve tool for all questions\n"
    system_prompt += "\n"
    system_prompt += "Write only improved questions. Write in Korean.\n"
    system_prompt += "</SYSTEM>"
